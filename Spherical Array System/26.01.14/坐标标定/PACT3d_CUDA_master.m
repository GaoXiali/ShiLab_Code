...
folder_path = 'I:\GXL\球阵系统\26.01.14\坐标标定\';
str_name = dir(fullfile(folder_path, '*_0.mat'));
[datax,DAQ_time_point] = func_3D_PACT_Data_Time_Read(folder_path,str_name(9).name);
...
T = 22.8; % 水温
V_M = 1479.5;
V_M_Range = 1470:0.5:1530; % 单声速迭代范围

% VM_out = waterSoundSpeed(T); % 外声速（水），单位 m/s 
VM_out = 1479.5; % 外声速（水），单位 m/s  
VM_out_Range = 1475:0.5:1499; % 外声速迭代范围
VM_in = 1479.5; % 内声速，单位 m/s  
...
step_length_y = 7; %mm 垂直相邻两帧位移距离

% 图像重建的尺寸设置
x_size = 30;
y_size = 35;
z_size = 20;
resolution_factor = 15; % 分辨率因子
center_x = 10;  % 中心坐标X
center_y = 0; % 中心坐标Y
center_z = 4;  % 中心坐标Z
    ...
    case 9 %单/双声速旋转平移复合
        ...
        for xframe = 1:step_x:Nframex
            ...
            for yframe = 1:step_y:Nframey %文件数量方向
                ...
                pa_data = -datax(:,:,1:1:end);%选取光声帧或超声帧
        ...
        %%
                ...
                static_frames = 1:Nframe;
                top_vals = maxk(corr_line, 1);%找出最大的20个值，并按降序排列
