...
% 椭球面位置需根据实验情况进行调整
%
% Author：Xiali Gao  Version：3.0  2025.12.30
%
% *************************************************************************
...
folder_path = 'E:\Summer Pear\Program\work\matlab\Photot Acoustic Imaging\球阵系统\25.12.24\旋转+平移\';
str_name = dir(fullfile(folder_path, '*_0.mat'));
[datax,DAQ_time_point] = func_3D_PACT_Data_Time_Read(folder_path,str_name(57).name);
...
%% 系统参数设置
reconstruct_mode = 10; % 1: 单声速CUDA重建; 2: 双声速CUDA重建; 3：内声速迭代 
                      % 4：外声速迭代; 5:单声速迭代 6:单声速旋转复合 7:双声速旋转复合 
                      % 8:单声速相干因子旋转复合 9:单声速二维平移复合 10：双声速旋转平移复合

% 声速设置
T = 22.8; % 水温
V_M = 1516.0;
V_M_Range = 1500:0.5:1530; % 单声速迭代范围

% VM_out = waterSoundSpeed(T); % 外声速（水），单位 m/s 
VM_out = 1516; % 外声速（水），单位 m/s  
VM_out_Range = 1475:0.5:1499; % 外声速迭代范围
VM_in = 1516; % 内声速，单位 m/s  
VM_in_Range = 1555:5:1699; % 内声速迭代范围


step_x = 1; %mm 平移复合位移数（文件数量方向）
step_y = 1; %mm 平移复合位移数(帧/文件数量方向）
step_length_x = 4; %mm 水平相邻两帧位移距离
...
theta_x = 0; % 以x轴为中心旋转 
theta_y = 0; % 以y轴为中心旋转 
theta_z = 42.6; % 以z轴为中心旋转 位移台与坐标轴夹角42.6
...
switch reconstruct_mode
    ...
    case 10 %单/双声速旋转平移复合
        ...
        % 当前程序设定采集帧率为10Hz，程序中包含未旋转帧过滤程序，默认过滤从开始采集后1.5s内的数据，确保复合数据均为旋转数据
        
        imgsize = size(Points_img,1:3);
        Ndata = size(str_name,1)/4;%采集数据组数
        Nframex = 6; %x方向扫描次数
        Nframey = floor(Ndata/Nframex); %y方向扫描次数
        pa_img_total = zeros(imgsize+[step_length_y*resolution_factor*(Nframey-1) ...
                                           step_length_x*resolution_factor*(Nframex-1) 0]);    
        ...
        axis equal tight; colormap gray; colorbar; set(gca, 'tickdir', 'out');
        ylabel('X'); xlabel('Z');title('ZX proj'); axis equal;set(gca, 'YDir', 'normal');
        subplot(132); h_img10_2 = imagesc(x_range_total, y_range_total, zeros(length(y_range_total), length(x_range_total))); % 注意尺寸
        axis equal tight; colormap gray; colorbar; set(gca, 'tickdir', 'out');
        ylabel('Y'); xlabel('X');title('XY proj'); axis equal;set(gca, 'YDir', 'normal');
        subplot(133); h_img10_3 = imagesc(z_range_total, y_range_total, zeros(length(y_range_total), length(z_range_total))); % 注意尺寸
        axis equal tight; colormap gray; colorbar; set(gca, 'tickdir', 'out');
        ylabel('Y'); xlabel('Z');title('ZY proj'); axis equal;set(gca, 'YDir', 'normal');
        
        for xframe = 1:step_x:Nframex

            for yframe = 1:step_y:Nframey %文件数量方向

                VM_out = VM_out-0.09; %水温降低的声速补偿，若实验使用冷水则关闭该补偿
                VM_in = VM_out;
                frame_idx = 1+((xframe-1)*Nframey+(yframe-1))*4;
        ...
        %%
                ...
                % 软件中触发速度11000对应0.811°，那么软件中触发速度10000可线性计算-改
                delta_angle = -11000*0.816/11000; %角度°
                static_Nframe = size(static_frames,2);
                ...
                for frame = 1:1:static_Nframe %注意这里的step*delta_angle为相邻复合角
                    ...
                    % 归一化并应用非线性增强
                    pa_img2 = pa_img1./total_angle_weight;
                    [GaussianMask, ~, ~] = generateGaussianMask({x_range, y_range}, 'Center', [5,2], 'Sigma', 8);
                    GaussianMask = GaussianMask.*ones(1,1,size(z_range,2));%z方向均匀
                    pa_img3 = pa_img2.*GaussianMask;
                ...
                current_weight = single(static_Nframe);%单组数据权重
                ...
                pa_img_display = pa_img_total ./ (pa_count_total + 0.1);%去除帧数不均对亮度的影响，避免除以0
                pa_img_total_2 = subplus(pa_img_display);
                imin=min(pa_img_total_2,[],"all");
                imax=max(pa_img_total_2,[],"all");

                % 更新 Figure 10 的数据
                set(h_img10_1, 'CData', squeeze(max(pa_img_display(:,:,:),[],1)));
                set(h_img10_1.Parent, 'CLim', [imin, imax]); 
                set(h_img10_2, 'CData', squeeze(max(pa_img_display(:,:,:),[],3)));
                set(h_img10_2.Parent, 'CLim', [imin, imax]); 
                set(h_img10_3, 'CData', squeeze(max(pa_img_display(:,:,:),[],2)));
                set(h_img10_3.Parent, 'CLim', [imin, imax]); 
                % drawEllipsoidOverlay(Ellipse);%显示双声速范围
        ...
        pa_img_total_2_cut = pa_img_total_2(200:end-100,1:end-200,140:end-200);
...
pa_img_deep = permute(pa_img_total_2, [1, 2, 3]);
pa_deep_xy = colorMIP(pa_img_deep);

file_name = ['pa_deep_xy_', num2str(frame), '.png'];
full_save_path = fullfile(folder_path, file_name);
imwrite(mat2gray(pa_deep_xy), full_save_path);
